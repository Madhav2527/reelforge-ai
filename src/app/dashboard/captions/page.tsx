...
import { PenTool, Copy, Sparkles, Crown, Lock } from "lucide-react"
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('/api/generate', {
        ...
        body: JSON.stringify({ niche, audience, mode: 'captions' }),
      })
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold font-outfit text-white mb-2">Captions & Hooks</h1>
          <p className="text-muted-foreground">Generate scroll-stopping captions to boost your engagement.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-1">
          <div className="glass-card p-6 rounded-2xl border border-white/10 space-y-4">
            <div>
              ...
              <input type="text" value={niche} onChange={(e) => setNiche(e.target.value)} placeholder="e.g. Real Estate" className="w-full bg-background border border-white/10 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-primary outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-muted-foreground mb-1.5">Audience</label>
              <input type="text" value={audience} onChange={(e) => setAudience(e.target.value)} placeholder="e.g. First-time buyers" className="w-full bg-background border border-white/10 rounded-lg px-4 py-2.5 text-white focus:ring-2 focus:ring-primary outline-none" />
            </div>
            <Button className="w-full" onClick={handleGenerate} disabled={isGenerating}>
              {isGenerating ? "Writing..." : usageCount >= FREE_LIMIT ? <><Lock className="mr-2" size={16}/> Upgrade to Generate</> : "Generate Captions"}
            </Button>
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          <div className="glass-card p-6 rounded-2xl border border-white/10 min-h-[400px] flex flex-col">
            ...
            {isGenerating ? (
              <div className="flex-1 flex flex-col items-center justify-center"><Sparkles className="animate-pulse text-primary mb-4" size={32} /><p className="text-muted-foreground">Writing captions...</p></div>
            ) : error ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50"><PenTool size={48} className="text-muted-foreground mb-4" /><p className="text-muted-foreground">Fill out the details to generate captions.</p></div>
            )}
          </div>
        </div>
      </div>
